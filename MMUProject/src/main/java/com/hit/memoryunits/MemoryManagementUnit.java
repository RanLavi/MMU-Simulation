...
public class MemoryManagementUnit 
{
	...
	public Page<byte[]>[] getPages(Long[] pageIds,boolean[] writePges) throws IOException
	{
		List<Long> returnedPages = new ArrayList<>();
		for(int i=0; i<pageIds.length; i++ )
			returnedPages.add(m_algo.getElement(pageIds[i]));
		...
		List<Long> pageIdsToHandle =  new ArrayList<>();
		for(int i=0; i<pagesNotInRam.size(); i++ )
			pageIdsToHandle.add(m_algo.putElement(pagesNotInRam.get(i), pagesNotInRam.get(i)));
